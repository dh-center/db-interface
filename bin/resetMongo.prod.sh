...
docker-compose -f docker-compose.prod.yml exec mongodb mongo db-interface --eval "db.locations.remove({}) && db.persons.remove({}) && db.addresses.remove({}) && db.changes.remove({}) && db.relations.remove({}) && db.relationtypes.remove({}) && quit()"
